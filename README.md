...
> **局域网访问说明**：请仅在受信任的家庭 / 办公网络中使用，并在系统防火墙中放行该端口。服务没有登录或加密传输；各设备的内容保存在自己的浏览器中，不会上传或写入运行服务的电脑。

### 浏览器本地配置

`data/resume.json` 只作为**只读默认配置**。设备第一次打开页面时，浏览器会读取这份默认配置并写入自己的 `localStorage`；之后编辑、拖拽排序和自动保存都只发生在该设备、该浏览器内，服务端不会按 IP 建档，也不接受写入请求。

- 换浏览器、换设备、清除该站点浏览数据后，会重新加载默认配置。
- 请使用顶部「导出」保存 JSON 备份；需要恢复时用「导入」。
- 默认配置更新只会影响首次访问或已清除本地数据的浏览器，已保存的浏览器数据不会被覆盖。
...
| 「导出」 | 下载当前浏览器内数据的 JSON 快照 |
| 「导入」 | 用 JSON 文件替换当前浏览器内的数据 |
| `Ctrl+S` | 立即保存到当前浏览器 |

编辑后 **900ms 自动保存**到当前浏览器的 `localStorage`，不会上传到服务端。清除浏览器站点数据前请先用「导出」备份。
...
```bash
node server.js --data "D:/我的简历/resume.json"   # 换一个只读默认配置文件
...
默认配置文件只在服务启动时缺失时自动创建；页面内的保存不会修改该文件。

## 目录结构

```
server.js            局域网静态托管 + 只读默认配置 API（零依赖，仅用 Node 内置模块）
...
data/resume.json     只读默认配置（已被 .gitignore 排除）
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/api/data` | 读取只读默认配置，供首次访问的浏览器初始化 |
| `PUT` | `/api/data` | 不支持（返回 405，调用方数据仅保存在浏览器） |
| `GET` | `/api/meta` | 读取默认配置的路径与 mtime |
